...
public class CommentServiceTests
{
    ...
    public async void SearchComment_WhenIdExists_ShouldReturnComment()
    {
        ...
        // Act
        CommentDto commentDtoReturned = await _commentService.FindCommentById(comment.Id);

        // Assert
        _commentRepositoryMock
            .Verify(_commentRepositoryMock => _commentRepositoryMock.GetByIdAsync(comment.Id), Times.Once);
        commentDtoReturned.Content.Should().Be(contentComment);
        commentDtoReturned.Author.Name.Should().Be(nameUser);
        commentDtoReturned.Post.Content.Should().Be(contentPost);
